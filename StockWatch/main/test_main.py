...
def assert_redirects(r, url):
    assert hasattr(
        r, "redirect_chain"
    ), "Response was not a redirect. Did you use follow=True?"
...
def test_login(client):
    ...
    r = client.post(
        "/login/",
        {"username": "testing@stockwatch.com", "password": "testing"},
        follow=True,
    )
    assert_redirects(r, "/")
...
def test_search_symbols_ordered(auth_client, symbol_search_example):
    responses.add(
        "GET", "https://www.alphavantage.co/query", json=symbol_search_example
    )
...
def test_search_symbol_vantage_error(auth_client):
    responses.add(
        "GET", "https://www.alphavantage.co/query", json={"Error Message": "Foo"}
    )
